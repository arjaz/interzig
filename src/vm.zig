    ...
    pub fn interpret(self: *VirtualMachine) !Value {
        ...
        while (true) : (frame.ip += 1) {
            ...
            switch (instruction) {
                ...
                .StoreLocal => |index| {
                    self.stack.items[index] = self.stack.items[self.stack.items.len - 1 + frame.stack_base];
                },
                .LoadLocal => |index| {
                    self.stack.appendAssumeCapacity(self.stack.items[index + frame.stack_base]);
                },
                .LoadConstant => |index| {
                    const v = self.constants.items[index];
