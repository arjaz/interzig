...
test "interpret return" {
    ...
    const mainFnIndex = try vm.addObject(try main.named_function(std.testing.allocator, "main", 0, 0));
...
test "interpret set global" {
    ...
    const globalNameIndex = try vm.addObject(try main.string_from_u8_slice(std.testing.allocator, "nice"));
...
test "interpret fn call" {
    ...
    const justTrueFnIndex = try vm.addObject(try main.named_function(std.testing.allocator, "justTrue", 0, 0));
...
test "interpret closure call with an open local upmain.Value" {
    ...
    const justTrueFnIndex = try vm.addObject(try main.named_function(std.testing.allocator, "justTrue", 0, 1));
    const justTrueFn = &vm.objects.items[justTrueFnIndex];
    const justTrueChunk = &justTrueFn.Function.chunk;
...
test "interpret closure call with an open non-local upvalue" {
    ...
    const justTrue2FnIndex = try vm.addObject(try main.named_function(std.testing.allocator, "justTrue2", 0, 1));
